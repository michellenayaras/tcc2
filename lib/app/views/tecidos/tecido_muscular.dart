...
class EstudoTecidosMuscular extends StatefulWidget {
  const EstudoTecidosMuscular({Key? key}) : super(key: key);

  @override
  // ignore: library_private_types_in_public_api
  _EstudoTecidosMuscularState createState() => _EstudoTecidosMuscularState();
}

class _EstudoTecidosMuscularState extends State<EstudoTecidosMuscular> {
  late Future<List<Map<String, dynamic>>> _data;

  @override
  void initState() {
    super.initState();
    _data = MyDatabase().getData('Tecido muscular');
  }
        ...
        builder: (context, snapshot) {
          if (snapshot.hasData) {
            ...
            final parts = text
                ...
                .splitMapJoin(
                  RegExp(
                      r'(Esse tecido apresenta células alongadas  \(.+?\):|Tecido muscular estriado esquelético|Tecido muscular estriado cardíaco|Tecido muscular não estriado ou liso)'),
                  onMatch: (m) => '\n${m.group(0)}',
                  onNonMatch: (n) => n,
                )
                .split('\n')
                ...
                .where((s) =>
                    ...
                    s.startsWith('Tecido muscular estriado esquelético') ||
                    s.startsWith('Tecido muscular estriado cardíaco') ||
                    s.startsWith('Tecido muscular não estriado ou liso'))
                .toList();
                  ...
                  return Column(
                    children: [
                      ...
                      Container(
                        padding: const EdgeInsets.all(12.0),
                        child: Image.asset(
                          'assets/images/tecido_muscular.jpg',
                          height: 200,
                          //width: 200,
                          fit: BoxFit.contain,
                        ),
