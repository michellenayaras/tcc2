...
class EstudoTecidosConjuntivo extends StatefulWidget {
  const EstudoTecidosConjuntivo({Key? key}) : super(key: key);

  @override
  // ignore: library_private_types_in_public_api
  _EstudoTecidosConjuntivoState createState() =>
      _EstudoTecidosConjuntivoState();
}

class _EstudoTecidosConjuntivoState extends State<EstudoTecidosConjuntivo> {
  late Future<List<Map<String, dynamic>>> _data;

  @override
  void initState() {
    super.initState();
    _data = MyDatabase().getData('Tecido conjuntivo');
  }
        ...
        builder: (context, snapshot) {
          if (snapshot.hasData) {
            ...
            final parts = text
                ...
                .splitMapJoin(
                  RegExp(
                      r'(Tecido Conjuntivo propriamente dito \(.+?\):|Tecido Hematopoiético|Tecido Cartilaginoso|Tecido Adiposo|Tecido Ósseo)'),
                  onMatch: (m) => '\n${m.group(0)}',
                  onNonMatch: (n) => n,
                )
                .split('\n')
